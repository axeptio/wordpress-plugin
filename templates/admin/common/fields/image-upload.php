<div x-data="imageUploadComponent({
	...
	<div class="flex items-center my-2">
		<input type="checkbox" id="use_<?php echo esc_attr($data->id); ?>"
			   x-model="useImage"
			   class="mr-2">
		<label for="use_<?php echo esc_attr($data->id); ?>" class="text-sm font-medium text-gray-700">
			<?php echo esc_html__('Use custom image', 'axeptio-wordpress-plugin'); ?>
		</label>
	</div>

	<div x-show="useImage">
		<div class="mt-2 flex items-center">
			<input type="hidden" :name="fieldName" :id="fieldId" x-model="imageUrl">
			<button type="button"
